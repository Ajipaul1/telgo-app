    ...
    return (
      <main style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", background: "linear-gradient(165deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)" }}>
        <div className="glass fade-in" style={{ width: "100%", maxWidth: 420, padding: "40px 28px", textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 12, color: "var(--text)" }}>Request Submitted!</h2>
          <p style={{ fontSize: 14, color: "var(--dim)", lineHeight: 1.7, marginBottom: 28 }}>Your access request has been sent to the admin. Once approved, you will receive an email with your login credentials.</p>
  ...
  return (
    <main style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", background: "linear-gradient(165deg, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)" }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", marginBottom: 6 }}>Request Access</h1>
        <p style={{ fontSize: 13, color: "var(--dim)" }}>Fill in your details. Admin will review and send you login credentials.</p>
      </div>

      <div className="glass fade-in" style={{ width: "100%", maxWidth: 420, padding: "28px" }}>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--dim)", marginBottom: 8 }}>Full Name</label>
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--dim)", marginBottom: 8 }}>Role Requested</label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {ROLES.map(r => (
                <button key={r} type="button" onClick={() => setRole(r)} style={{ padding: "10px 8px", borderRadius: 12, border: `1px solid ${role === r ? "var(--violet)" : "var(--border)"}`, background: role === r ? "rgba(124,58,237,0.08)" : "var(--surface)", color: role === r ? "var(--violet)" : "var(--muted)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "Outfit, sans-serif", textTransform: "capitalize", transition: "all 0.2s" }}>
                  {r}
                </button>
              ))}
            </div>
          ...
          {state === "error" && (
            <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 12, padding: "12px", fontSize: 13, color: "#dc2626" }}>{msg}</div>
          )}
      ...
      <a href="/login" style={{ marginTop: 20, fontSize: 13, color: "var(--muted)", textDecoration: "none" }}>← Back to Login</a>
    </main>
  );
}
