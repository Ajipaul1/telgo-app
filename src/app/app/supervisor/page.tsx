  ...
  return (
    <div style={{ minHeight:"100dvh", background:"#060912", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px" }}>
      <div className="glass fade-in" style={{ width:"100%", maxWidth:420, padding:"36px 28px", textAlign:"center" }}>
        <div style={{ width:64, height:64, borderRadius:"50%", background:"linear-gradient(135deg,#0e7490,#06b6d4)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 20px" }}>
        ...
        <h2 style={{ fontSize:22, fontWeight:800, color:"#f1f5f9", marginBottom:8 }}>Welcome, {user?.fullName ?? "Supervisor"}!</h2>
        <p style={{ fontSize:13, color:"#64748b", marginBottom:6 }}>{user?.email}</p>
        <span style={{ display:"inline-block", background:"rgba(6,182,212,0.15)", color:"#67e8f9", border:"1px solid rgba(6,182,212,0.25)", borderRadius:8, padding:"4px 14px", fontSize:12, fontWeight:700, textTransform:"uppercase", marginBottom:28 }}>Supervisor</span>
        <p style={{ fontSize:14, color:"#475569", marginBottom:32 }}>Your dashboard is being built. Check-in, tasks and reports are coming in the next update.</p>
